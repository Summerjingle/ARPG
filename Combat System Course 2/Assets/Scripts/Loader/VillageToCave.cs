   ...
   private void OnTriggerExit(Collider other)
    ...
    private void Update()
    {
        if (isInTrigger && Input.GetKeyDown(KeyCode.E) && !isTeleporting)
        {
            if (eTips != null)
                eTips.SetActive(false);
            Vector3 targetPosition = new Vector3(25.908f, 4.8f, -60.088f);  // 目标位置
            Quaternion targetRotation = Quaternion.Euler(0, 128.346f, 0);  // 目标旋转
    ...
    //    // 等待动画播放（如果有的话）
    //    if (playerAnim != null)
    //        yield return new WaitForSeconds(5.0f); // 调整时间为动画长度
